...
bsModal <- function(id, title, trigger, ..., size, 
    footer = NULL, close.button = TRUE, width = NULL) {
    ...
    if (close.button) {
        footer <- 
            shiny::tagAppendChild(footer, 
                htmltools::tagList(shiny::tags$button(type = "button", 
    ...
    bsTag <- shiny::tags$div(class = size, 
        ...
        shiny::tags$button(type = "button", class = "close", 
            "data-dismiss" = "modal", shiny::tags$span(shiny::HTML("&times;"))),
        shiny::tags$h4(class = "modal-title", title)),
        shiny::tags$div(class = "modal-body", list(...)),
        shiny::tags$div(class = "modal-footer", footer)))

    if (!is.null(width)) {
        bsTag <- addAttribs(bsTag, style = paste0("width: ", width, 
        " !important;"))
    }
